...
function extractStackFromError(error) {
    return utils_1.createStackParser(stack_parser_1.nodeStackParser)(error.stack || '');
}
...
function exceptionFromError(error) {
    ...
    var frames = extractStackFromError(error);
    if (frames.length) {
 ...
 * @hidden
 */
function eventFromError(exception, hint) {
...
function eventFromMessage(options, message, level, hint) {
    ...
    if (options.attachStacktrace && hint && hint.syntheticException) {
        var frames_1 = extractStackFromError(hint.syntheticException);
        if (frames_1.length) {
